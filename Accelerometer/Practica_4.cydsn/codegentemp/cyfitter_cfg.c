...
#define BS_CYDEV_CLKDIST_ACFG0_CFG0_VAL ((const uint8 CYFAR *)0x48000058u)
...
uint8 CYXDATA * const CYCODE variables__addrTable[6] = {
...
/* This is an implementation detail of the AMux. Code that depends on it may be
   incompatible with other versions of PSoC Creator. */
const uint8 CYCODE variables__maskTable[6] = {
	0x80u, 0x08u, 
	0x40u, 0x04u, 
	0x20u, 0x02u, 
};
...
*
*******************************************************************************/
void variables_Set(uint8 channel)
{
	if (channel < 3)
	{
		channel += channel;
		*variables__addrTable[channel] |= variables__maskTable[channel];
		channel++;
		*variables__addrTable[channel] |= variables__maskTable[channel];
	}
}
...
*
*******************************************************************************/
void variables_Unset(uint8 channel)
{
	if (channel < 3)
	{
		channel += channel;
		*variables__addrTable[channel] &= (uint8)~variables__maskTable[channel];
		channel++;
		*variables__addrTable[channel] &= (uint8)~variables__maskTable[channel];
	}
}
